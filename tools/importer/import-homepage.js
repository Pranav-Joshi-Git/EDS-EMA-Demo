...
// TRANSFORMER REGISTRY - cleanup runs first, sections after (adds <hr> + metadata)
const transformers = [
  cleanupTransformer,
...
export default {
  transform: (payload) => {
    ...
    // Root ("/") and empty pathnames map to "/index" (EDS convention); an empty
    // path would otherwise trigger a path.resolve()/process.cwd() call in the
    // importer runtime.
